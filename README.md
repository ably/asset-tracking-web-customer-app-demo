# Ably Asset Tracking Demo: Web Customer
...
being the tracking of a delivery being made to that customer, where that customer is using a Web browser.
...
If you are running the app for the first time, then install the dependencies run:
```bash
npm install
```

Also you will need to make sure you build the ably asset tracking [SDK](https://github.com/ably/ably-asset-tracking-js) before you can run the demo app, in the **root directory** of the SDK repository run:
```bash
npm install
npm run build
```

Then in the `server.js` of this demo app, update the server to use the generated `dist` folder from the SDK build:

```
app.use(express.static('<path to SDK>/dist'));
```

To run the web demos, run:
...
public/Google** - Google maps implementation  
public/Mapbox** - Mapbox maps implementation
